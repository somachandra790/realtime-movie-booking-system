...
export const CinemasBody = ({ filters }) => {
  ...
  const date_formated_cinemas_data = useSelector(
    (state) => state.cinemas.date_formated_cinemas_data
  );
  const dispatch = useDispatch();
  let filteredData = date_formated_cinemas_data ?? cinemas_data;
            ...
            <div className={styles.container__card__info}>
              <div className={styles.container__card__info__options}>
                <div style={{ color: "#49BA8E" }}>
                  <VscDeviceMobile />
                  <span>{cinema?.room?.name}</span>
                </div>
                <div style={{ color: "#FFB23F" }}>
                  <IoFastFoodOutline />
                  <span>
                    {cinema?.projectionDate?.hours}:
                    {cinema?.projectionDate?.minutes}{" "}
                    {cinema?.projectionDate?.hours >= 12 ? "PM" : "AM"}
                  </span>
                </div>
              </div>
